namespace JoinUsAPIv3.Migrations
{
    ...
    internal sealed class Configuration : DbMigrationsConfiguration<JoinUsAPIv3.Models.JoinUsAPIv3Context>
    {
        ...
        protected override void Seed(JoinUsAPIv3.Models.JoinUsAPIv3Context context)
        {
            ...
            var eventChess = new Event() { Id = 2, Title = "Concour d'échecs ChessMaster 2.0", Address = "Rue des Carmes, 77, 5000 Namur", CreatorId = 1, Date = new DateTime(2017, 8, 18), Description = "Notre tournoi d'échecs par catégories ELO est ouvert. Venez disputer les meilleures parties d'échecs!" };
            eventChess.Categories.Add(boardGamesCategory);
            eventChess.Tags.Add(tagChess);
            eventChess.Tags.Add(tagContest);
            var eventHS = new Event() { Id = 3, Title = "Hearthstone Arena", Address = "Rue de Bruxelles, 51, 5000 Namur", CreatorId = 2, Date = new DateTime(2017, 8, 18), Description = "Le petit bitu vous invite à participer à son tournoi de Hearthstone! Ce tournoi est à élimination directe. jusqu'à 200 euros de chèque pour du matériel informatique reviendront au vainqueur!" };
            eventHS.Categories.Add(videoGamesCategory);
            eventHS.Tags.Add(tagHS);
            eventHS.Tags.Add(tagContest);
            var eventExpo = new Event() { Id = 4, Title = "Exposition Picasso", Address = "Place du manège, 1, 6000 Charleroi", CreatorId = 1, Date = new DateTime(2017, 8, 21), Description = "Le Palais des Beaux-Arts vous invite à venir découvrir sa nouvelle exposition concernant Pablo Picasso!" };
            eventExpo.Categories.Add(cultureCategory);
            eventExpo.Tags.Add(tagPicasso);
            eventExpo.Tags.Add(tagExpo);
            var eventEng = new Event() { Id = 5, Title = "TED talk:\"How new Technologies affect social relationships\"", Address = "Rue de Bruxelles, 61,5000 Namur", CreatorId = 1, Date = new DateTime(2017, 10, 17), Description = "TED Talk about how new technologies affect our behavior towards other people" };
            eventEng.Categories.Add(cultureCategory);
            eventEng.Categories.Add(languagesCategory);
            eventEng.Tags.Add(tagEnglish);
            var eventAth = new Event() { Id = 6, Title = "Petite course dans Namur", Address = "Rue des Carmes, 77, 5000 Namur", CreatorId = 1, Date = new DateTime(2017, 11, 18), Description = "J'organise une course dans Namur. Point de départ chez moi (voir adresse). Contactez-moi pour plus d'infos" };
            eventAth.Categories.Add(sportCategory);
            eventAth.Tags.Add(tagAth);
            var eventMaths = new Event() { Id = 7, Title = "Remédiation Maths", Address = "Rue des Carmes, 77, 5000 Namur", CreatorId = 1, Date = new DateTime(2017, 6, 7), Description = "Vous avez des problèmes dans le domaine des mathématiques? Je vous invite à me rendre visite, je me ferai un plaisir de vous aider" };
